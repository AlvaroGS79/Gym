...
def generate_professional_view(m, diet_type, goal):
    ...
    with tab_suples:
        ...
        with s1:
            st.success("**Creatina**")
            st.write("5g diarios. Mejora fuerza.")
        with s2:
            st.success("**Proteína Whey**")
            st.write("1 scoop si te falta proteína.")
        with s3:
            st.warning("**Omega 3 / Cafeína**")
            st.write("Según objetivo y salud.")

    with tab_compra:
        st.table(pd.DataFrame({
            "Categoría": ["Proteína", "Carbos", "Grasas"],
            "Alimentos": ["Pollo, Pescado, Huevos", "Avena, Arroz, Patata", "Aceite, Nueces"]
        }))
...
if sel == "Dashboard":
    ...
    if not res.data: st.warning("Configura tu perfil en Ajustes.")
        ...
        p = res.data[0]
        # CORRECCIÓN AQUÍ: Separamos el selectbox del header
        goal = st.selectbox("Selecciona tu objetivo", ["Perder Grasa", "Mantener", "Ganar Músculo"])
        st.header(f"Plan de Alimentación para: {goal}")
